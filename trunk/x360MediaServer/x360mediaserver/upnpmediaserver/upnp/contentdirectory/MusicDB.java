...
import x360mediaserver.upnpmediaserver.upnp.formats.identifiers.ExtensionIdentifier;
import x360mediaserver.upnpmediaserver.upnp.formats.streamers.External;
import x360mediaserver.upnpmediaserver.upnp.formats.streamers.Native;
import x360mediaserver.upnpmediaserver.upnp.formats.streamers.StreamExternal;
import x360mediaserver.upnpmediaserver.upnp.formats.streamers.StreamNative;
import x360mediaserver.upnpmediaserver.upnp.formats.tags.EntaggedTagger;
import x360mediaserver.upnpmediaserver.upnp.formats.tags.FileTagger;
import x360mediaserver.upnpmediaserver.upnp.formats.tags.MP4Tagger;
import x360mediaserver.upnpmediaserver.upnp.items.Album;
import x360mediaserver.upnpmediaserver.upnp.items.Artist;
import x360mediaserver.upnpmediaserver.upnp.items.Container;
import x360mediaserver.upnpmediaserver.upnp.items.FileItem;
import x360mediaserver.upnpmediaserver.upnp.items.Media;
import x360mediaserver.upnpmediaserver.upnp.items.Playlist;
import x360mediaserver.upnpmediaserver.upnp.items.Song;
import x360mediaserver.upnpmediaserver.upnp.items.Stream;
import x360mediaserver.upnpmediaserver.upnp.items.Video;
...
public class MusicDB
{
    ...
    public void reset()
    {
        ...
        songSet = new TreeSet<Song>(new Comparator<Song>()
        {
        ...
        videoSet = new TreeSet<Video>(new Comparator<Video>()
        {
    ...
    private void initFormats()
    {
        ...
        // mp3
        NewFormat newformat = new NewFormat(new ExtensionIdentifier("mp3"), new EntaggedTagger(),
                                            formatHandler, new Native(), null, null, null, true,
                                            false);
        newformat.setITunesKind("MPEG audio file");
        formatHandler.addFormat(newformat);
        // aac
        newformat = new NewFormat(new ExtensionIdentifier("m4a"), new MP4Tagger(), formatHandler,
                                  new External("m4amp3", formatHandler),
                                  new External("m4apcm", formatHandler), null, null, true, false);
        newformat.setITunesKind("AAC audio file");
        formatHandler.addFormat(newformat);
        // flac
        formatHandler.addFormat(new NewFormat(new ExtensionIdentifier("flac"),
                                              new EntaggedTagger(), formatHandler, null,
                                              new External("flacpcm", formatHandler), null, null,
                                              true, false));
        // ogg
        formatHandler.addFormat(new NewFormat(new ExtensionIdentifier("ogg"), new EntaggedTagger(),
                                              formatHandler, new External("oggmp3", formatHandler),
                                              null, null, null, true, false));
        // wma
        formatHandler.addFormat(new NewFormat(new ExtensionIdentifier("wma"), new EntaggedTagger(),
                                              formatHandler, null, null, new Native(), null, true,
                                              false));
        formatHandler.addFormat(new NewFormat(new ExtensionIdentifier("wmv"), new FileTagger(),
                                              formatHandler, null, null, new Native(), null, false,
                                              true));
        formatHandler.addFormat(new NewFormat(new ExtensionIdentifier("avi"), new FileTagger(),
                                              formatHandler, null, null,
                                              new External("aviwmv", formatHandler), null, false,
                                              true));

        streamFormats = new StreamFormat[3];
        // mp3 streams
        streamFormats[0] = new StreamFormat(formatHandler, new StreamNative(), null, null, null,
                                            true, false);
        streamFormats[1] = new StreamFormat(formatHandler, null, null, new StreamNative(), null,
                                            true, false);
        streamFormats[2] = new StreamFormat(formatHandler, new StreamExternal("playstreamasmp3",
                                                                              formatHandler),
                                            new StreamExternal("playstreamaspcm", formatHandler),
    ...
    protected boolean addSong(Song newsong)
    {
        ...
        if (artist == null)
        { // if the artist wasn't found in the library
    ...
    private boolean addVideo(FileItem file)
    {
        ...
        System.out.println("Video item created with title:" + newVideo.getTitle() + " " +
                           newVideo.getId());
    ...
    {
        if (album == null)
            album = ""; // handle no album
        String albumName = album.toLowerCase().trim();
        return albumMap.get(albumName);
    }

    // function to find artist in library, should probably do stuff like matching variations on name
    // e.g The, capitals etc
    ...
    { // add file to the database
        ...
        if (file.isDirectory())
        { // if it is a directory then add its children
            try
            {
                ...
                for (int i = 0; i < filearray.length; i++ )
                {
                    if (filearray[i].canRead())
                        addFile(filearray[i]); // if we can't read the file then ignore it
                }
            }
            catch (Exception e)
            {
        ...
        { // if its a file
            ...
            { // drop annoying hidden mac files
...
// }
    ...
    public void performFullSearch(ContentNodeList contentlist, String searchstring)
    {
        if (searchstring.contains("upnp:artist"))
        {
            ...
            if (artist != null)
                contentlist.addAll(artist.getAlbumContentList());
        }

    }

    public void getHackSearchList(ContentNodeList contentlist, String containerID,
                                  String searchCriList, String serverAddress)
    {
