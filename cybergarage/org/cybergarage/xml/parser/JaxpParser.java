...
//		Debug.message("[" + rank + "] ELEM : " + domNodeName + ", " + domNodeValue + ", type = " + domNodeType + ", attrs = " + arrrsLen);
	...
	public Node parse(InputStream inStream) throws ParserException
	{
		...
		try {
			...
			/*
			ServiceReference ref = context.getServiceReference( DocumentBuilderFactory.class.getName() );
			if ( ref == null ){
				context.stop();
				log.println("Unable to find XML Parser Service");
				return null;
			}
			builder =(DocumentBuilder) context.getService(ref);
			*/
